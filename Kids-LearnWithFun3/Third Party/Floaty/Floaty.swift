...
open class Floaty: UIView {
    ...
    weak var floatingActionButtonDelegate : FloatingActionButtonProtocol?
    ...
    @objc open var showLocationIcon = false
    /**
     ...
     */
    @IBInspectable
    @objc open var rotationDegrees: CGFloat = -180
...
//    @objc open var buttonColor: UIColor = UIColor(red: 73/255.0, green: 151/255.0, blue: 241/255.0, alpha: 1)
    @objc open var buttonColor: UIColor = UIColor.clear

    /**
     Button shadow color.
     */
    @IBInspectable
    @objc open var buttonShadowColor: UIColor = UIColor.clear
    ...
    var settingFABParameter : [String:Any] = [:]

    /**
     Plus icon color inside button.
     */
    @IBInspectable
    @objc open var plusColor: UIColor = UIColor.clear
    
    /**
     Background overlaying color.
     */
    @IBInspectable
    @objc open var overlayColor: UIColor = UIColor.clear
    
    /**
     The space between the item and item.
     */
    @IBInspectable
    @objc open var itemSpace: CGFloat = 19
     var itemSpaceFirstItem: CGFloat = 15
    /**
     Child item's default size.
     */
    @IBInspectable
    @objc open var itemSize: CGFloat = 30 {
        didSet {
     ...
     */
    @IBInspectable
    @objc open var itemButtonColor: UIColor = UIColor.clear
     ...
     */
    @IBInspectable
    @objc open var itemShadowColor: UIColor = UIColor.clear
...
//                floatingActionButtonDelegate!.rotateSettingButtonRotate()
    ...
    fileprivate func setButtonImage() {
        ...
        buttonImageView.tintColor = UIColor.gray
...
extension Floaty {
    ...
    fileprivate func popAnimationWithOpen(group: DispatchGroup) {
        ...
        for (index, item) in items.enumerated() {
            if item.isHidden == true { continue }
            if index == 0 {
                itemHeight += item.size + itemSpace + itemSpaceFirstItem
            } else {
